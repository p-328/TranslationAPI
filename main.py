...
def loadLangs():
  ...
  with open("compat.txt") as f:
    ...
    for _, value in m.copy().items():
      m[value] = value
    # return dictionary of language and language mapping
    return m
...
def index():
  ...
  with open("routes.txt", "r") as routes:
    ...
    with open("descriptions.txt", "r") as descs:
      description_texts = descs.read().split('----------------------------------------------------------------')
  ...
  # Returns JSON of documentation for this API.
...
def translate(from_lang, to_lang):
  ...
  if 'text' not in json_data:
    return jsonify({'error': 'Missing text JSON field!', 'mappings': lang_maps})
